...
class WADataManager: NSObject {
    ...
    func setDesktopImage(_ url: URL, model: Any? = nil) {
        ...
        if isNotification {
            let title = result ? "壁纸切换成功" : "壁纸切换失败"
    ...
    private func postNotification(with message: String, model: WADownloadModel?) {
        ...
        let categoryId = "com.liyb.Wallpaper.category"
        ...
        let request = UNNotificationRequest.init(identifier: "com.liyb.Wallpaper.notification", content: content, trigger: nil)
    ...
    private func configAppStartUp(_ isStartUp: Bool) {
        let helperAppIdentifier = "com.liyb.WallpaperHelper"
